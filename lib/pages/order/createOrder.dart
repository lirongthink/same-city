...
class CreateOrder extends StatefulWidget {
  CreateOrder({Key key, this.goodId}) : super(key: key);

  final String goodId;

  @override
  _CreateOrderState createState() => _CreateOrderState(goodId);
}

class _CreateOrderState extends State<CreateOrder> {
  
  _CreateOrderState(this.goodId);
  final String goodId;
  ...
  _getOrderInfo() async{
    ...
    Map<String,dynamic> obj = {
        'status':'已下单',
        'good':this.goodId,
        'buyer':Provider.of<UserIdModel>(context,listen: false).userId,
        'seller':res.data['data']['user']['_id']
    };

    var orderRes = await https('post','order',obj);
    if (orderRes.data['errno'] == 1000) {
      Toast.toast(context,msg: "订单创建成功",position: ToastPostion.center);
      var orders = await https('get','order/${orderRes.data['data']['_id']}',{});
      setState(() {
        orderModel = OrderModel.fromJSON(orders.data['data']);
      });
      await https('delete','goods/$goodId',{});
    } else {
      Toast.toast(context,msg: "订单创建失败，请返回重试",position: ToastPostion.center);
    }
